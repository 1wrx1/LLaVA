...
'''if download == 1:
    #os.mkdir('/grp01/saas_lqqu/runxi/llava/data/OCR/images')
    for k in tqdm(data.keys()):
        ext=os.path.splitext(data[k]['imageURL'])[1]
        outputFile='/grp01/saas_lqqu/runxi/llava/data/OCR/images/%s%s'%(k,ext)
        #pdb.set_trace()
        ureq.urlretrieve(data[k]['imageURL'],outputFile)    '''

if download == 1:
    # os.mkdir('/grp01/saas_lqqu/runxi/llava/data/OCR/images')  # Ensure the directory exists
    total_files = len(data)  # Get the total number of files to download

    # Create the output directory if it doesn't exist
    output_dir = '/grp01/saas_lqqu/runxi/llava/data/OCR/images'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for k in tqdm(data.keys(), desc="Downloading images", total=total_files, unit="file"):
        ext = os.path.splitext(data[k]['imageURL'])[1]
        outputFile = os.path.join(output_dir, '%s%s' % (k, ext))
        ...
        if not os.path.exists(outputFile):
            try:
                ...
                print(f"Error downloading {data[k]['imageURL']}: {e}")
        else:
            pass
            # If the file already exists, skip the download
            #print(f"File {outputFile} already exists, skipping download.")
